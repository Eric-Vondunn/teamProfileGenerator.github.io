    ...
    validate: (value) =>
      new Promise((resolve) => {
        setTimeout(
          () =>
            resolve(
              (!Number.isNaN(Number(value)) && value.length === 4) ||
                "Your ID has 4 digits"
            ),
          50
        );
      }),
    ...
    validate: (value) =>
      new Promise((resolve) => {
        setTimeout(
          () =>
            resolve(
              (!Number.isNaN(Number(value)) && value.length === 3) ||
                "Your Office Number has 3 digits"
            ),
          50
        );
      }),
          ...
          () =>
            resolve(
              (!Number.isNaN(Number(value)) &&
                value.length === 4 &&
                !idArray.includes(value)) ||
                "This ID is incorrect or belongs to someone else. Correct ID has 4 digits"
            ),
          50
        );
      }),
...
function addEngineer() {
  inquirer
    .prompt([questions[5], questions[6], questions[7], questions[8]])
    .then((answers) => {
...
function addIntern() {
  inquirer
    .prompt([questions[9], questions[10], questions[11], questions[12]])
    .then((answers) => {
